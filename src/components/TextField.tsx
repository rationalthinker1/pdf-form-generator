    ...
    <div className={[baseContainerClassName, containerClassName].filter(Boolean).join(' ')}>
      ...
      >
        {defaultValue
          ...
          : label
            ? <span className="text-gray-400 italic print:hidden">{label}</span>
            : null
        }
      </div>
      <input
        ...
        value={defaultValue}
        onChange={onChange}
        className="absolute inset-0 h-full w-full px-2 py-1 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-blue-400 print:hidden bg-blue-50"
      />
    </div>
  )
}
